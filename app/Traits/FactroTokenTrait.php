...
trait FactroTokenTrait
{
    public static function getAccessToken()
    {
        $apiConfiguration = FactroConfigurationService::apiConfiguration();
        ...
        if ($response->status() !== 200) {
            if (array_key_exists('error', $access_token) && $access_token['error'] === 'invalid_client') {
                throw new Exception('Error al autenticarse con la API de Factro');
            } else {
                throw new Exception('Error al obtener el token de acceso de Factro');
            }
        }
